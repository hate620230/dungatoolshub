...
const ToolListItem = ({ name, description, slug }: ToolListItemProps) => (
  <Link to={`/tool/${slug}`} className="group flex cursor-pointer items-center gap-3 rounded-lg border border-border bg-card px-4 py-3 transition-all duration-200 hover:border-primary/30 hover:shadow-md">
...
const CategorySection = ({ icon: Icon, label, emoji, color, tools, toolCount }: CategorySectionProps) => (
  <section className="mb-16">
    <div className="mb-6 flex items-center gap-3">
      ...
      <div>
        <h2 className="font-heading text-2xl font-bold text-foreground">{emoji} {label}</h2>
